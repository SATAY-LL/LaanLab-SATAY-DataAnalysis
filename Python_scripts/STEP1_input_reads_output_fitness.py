...
#filepath_and_name = r'C:\Users\floor\OneDrive\Documenten\MASTER\MEP\codes\LaanLab-SATAY-DataAnalysis\Python_scripts\Processed_data_WT1.wig'
    list = []
  ...
  #use imput to find n and m
    ...
    ni = data['#reads'].to_numpy() #array with reads for every insertion
    Ni = np.zeros(m)
    Ci = np.zeros(m)
    gf = np.zeros(m)
    fitness = np.zeros(m)
...
#forloop to compute growthfactor and fitness of insertions, relative to HO locus 
    ...
    #extract growthfactor of HO locus:
        if (data.at[i, 'chromosome']  == 'chrIV') & (46270 < data.at[i, 'tn start position']) & (data.at[i,'tn start position'] <  48031) :
            gfHO.append(gf[i])
    ...
    gfHO = statistics.mean(gfHO) #use this line if average of HO locus is taken after calculations of gf 

...
def reads_normalization(dna_df2, len_chr, normalization_window_size, wig_file):
    ...
    read_density_chromosome = sum([nc.Nreadsperbp for nc in dna_df2.itertuples() if nc.Feature_type == None])
    read_density_windows = np.ones(len(window_edge_list)-1)
    window_start = 0
    i = 0
    for window_end in window_edge_list[1:]:
        read_density = sum([nc.Nreadsperbp for nc in dna_df2.itertuples() if window_start <= nc.position[0] < window_end and nc.Feature_type == None])
...
#            read_density_windows.append(sum([nc[14] for nc in dna_df2.itertuples() if window_start <= nc[6][0] < window_end and nc[4] == None]))
    ...
    for row in dna_df2.itertuples():
        ...
        if not row.Feature_type == None and row.Feature_type.startswith('Gene'):
            norm_reads_central80p_list.append(row.Nreads_gene_central80p * (1/row.Nbasepairs*0.8) * ((10**6)/(total_reads_in_genome)) * (read_density_chromosome/read_density_windows[read_density_windows_index]))
        else:
            norm_reads_central80p_list.append(row.Nreads_gene_central80p * (1/row.Nbasepairs*1.0) * ((10**6)/(total_reads_in_genome)) * (read_density_chromosome/read_density_windows[read_density_windows_index]))
    ...
    for row in dna_df2.itertuples():
        if row.Feature_type == None:
            readsperbp_noncoding[window_index_list[i]] += row.Nreadsperbp_normalized
    ...
    normbync_reads_list = []
    for row in dna_df2.itertuples():
        normbync_reads_list.append(row.Nreadsperbp_normalized / avg_reads_noncoding[int(row.position[0] / window_length)])
    dna_df2['Nreadsperbp_normalized_byNCregions'] = normbync_reads_list
